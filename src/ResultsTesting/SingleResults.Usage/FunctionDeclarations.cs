...
public static class FunctionDeclarations
{
    ...
    public static Task<SingleResult<int>> IncrementAsync(int target) => Task.FromResult(SingleResult<int>.FromValue(target + 1));
    public static Task<SingleResult<int>> DoubleAsync(int target) => Task.FromResult(SingleResult<int>.FromValue(target * 2));
    public static Task<SingleResult<int>> TripleAsync(int target) => Task.FromResult(SingleResult<int>.FromValue(target * 3));
    public static Task<SingleResult<int>> AddAsync(int target1, int target2) => Task.FromResult(SingleResult<int>.FromValue(target1 + target2));
    public static Task<SingleResult<int>> DivideAsync(int numerator, int denominator) =>
        Task.FromResult((numerator, denominator) switch
        {
            (_, 0) => SingleResult<int>.FromException(new ApplicationException("can not divide by 0")),
